...
A Python script to process existing JPG images in a folder with a multimodal model, extracting content as markdown. This tool is designed to handle images that contain tabular and textual information, and output them in a structured markdown format.

## Key Features
- **Image Processing**: Processes existing JPG files in the specified folder.
...
def main():
    src_directory = "./data"  # Folder containing the images
    output_directory = "./output"
    model_name = 'llama3.2-vision:11b-instruct-q8_0'

    # Step 1: Gather all JPG image files in the source directory
    image_files = [os.path.join(src_directory, file) for file in os.listdir(src_directory) if file.endswith(".jpg")]
